...
func (psql *ProfileServiceSQL) GetProfileById(id string) (*model.Profile, error) {
	...
	row := psql.DB.QueryRow("SELECT id, username, displayname, isChannel FROM profile WHERE id = $1", id)
...
func (psql *ProfileServiceSQL) GetMultipleProfiles(amount int) ([]*model.Profile, error) {
	...
	rows, err := psql.DB.Query("SELECT id, username, displayname, isChannel FROM profile ORDER BY RANDOM() LIMIT $1", amount)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(&profile.ID, &profile.Username, &profile.Displayname, &profile.IsChannel)
